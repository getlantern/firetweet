...
public final class Firetweet implements FiretweetConstants {
    ...
    public static ParcelFileDescriptor getCachedImageFd(final Context context, final String url) {
        ...
        final Uri.Builder builder = FiretweetDataStore.CachedImages.CONTENT_URI.buildUpon();
    ...
    public static String getCachedImagePath(final Context context, final String url) {
        ...
        final Uri.Builder builder = FiretweetDataStore.CachedImages.CONTENT_URI.buildUpon();
        builder.appendQueryParameter(QUERY_PARAM_URL, url);
        final Cursor cur = resolver.query(builder.build(), FiretweetDataStore.CachedImages.MATRIX_COLUMNS, null, null, null);
        if (cur == null) return null;
        try {
            if (cur.getCount() == 0) return null;
            final int path_idx = cur.getColumnIndex(FiretweetDataStore.CachedImages.PATH);
            cur.moveToFirst();
    ...
    public static ParcelFileDescriptor getCacheFileFd(final Context context, final String name) {
        ...
        final ContentResolver resolver = context.getContentResolver();
        final Uri.Builder builder = FiretweetDataStore.CacheFiles.CONTENT_URI.buildUpon();
    ...
    public static FiretweetSharedPreferences getSharedPreferences(final Context context) {
        if (context == null) return null;
        return new FiretweetSharedPreferences(context);
    }
    ...
    public static boolean isPermissionGranted(final Context context) {
        ...
        final Cursor c = resolver.query(FiretweetDataStore.Permissions.CONTENT_URI, null, null, null, null);
        if (c == null) return false;
        try {
            c.moveToFirst();
            final int idxPackageName = c.getColumnIndex(FiretweetDataStore.Permissions.PACKAGE_NAME), idxPermissions = c
                    .getColumnIndex(FiretweetDataStore.Permissions.PERMISSION);
            while (!c.isAfterLast()) {
                if (pname.equals(c.getString(idxPackageName))) {
                    final String[] permissions = parsePermissions(c.getString(idxPermissions));
                    return FiretweetArrayUtils.contains(permissions, required);
                }
                c.moveToNext();
            }
